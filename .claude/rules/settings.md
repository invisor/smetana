...
the installed CLI's `model/list` app-server method when the Settings window or Project settings dialog opens. The shipped Codex table is
...
from `agents_catalog` read once at startup. Codex is separately refreshed through `codex_models` whenever
the Settings window or Project settings dialog opens; failure retains last-good and an unknown saved slug stays unavailable until a
