...
the board is drawn, `git`, the one thing the app does on its own initiative, `window`, whether the
main window opens where it was left, and `notifications`,
...
the four `kanban` fields, `git.autoFetch`, `window.restoreGeometry` and the two `notifications`
