...
The list has a **second reader outside this panel**: `dirtyCount`, the scope bar's first counter
(`.claude/rules/git-head.md`). It is deliberately nothing more than `tree.changes.length` — every
kind, staged and unstaged and untracked and conflicted alike — so that the number in the bar is the
number of rows here and can be checked by looking rather than by counting. It is `null` and never `0`
