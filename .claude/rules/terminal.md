...
surfaces. Its Run brief is held until that structured runtime is admitted, then
written once through the private PTY input; a failed admission kills it without
delivering a brief. Closing a Crew row therefore uses `crew_clear`, not
`terminal_remove`.
...
conversation. Since smetana-gb7f.2 Codex takes the same road for the same set of intents; only an
unsupported harness keeps this one. Which of the two a press takes is `canDrive` in the conversation store, asked once
...
writers that are not `showAgentTab` are these restores: each writes the field directly **because going through
