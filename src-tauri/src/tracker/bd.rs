...
///
/// Используется задачей 5 (инициализация после проверки каталога) — атрибут
/// временный и будет снят, когда воркер вызовет `Bd::init()`.
#[allow(dead_code)]
...
impl Bd {
    ...
    ///
    /// Используется задачей 5 (инициализация после проверки каталога) — атрибут
    /// временный и будет снят, когда воркер начнёт вызывать `init` после проверки.
    #[allow(dead_code)]
