...
mod tests {
    ...
    /// A field removed from this schema leaves its key behind in every file
    /// already on a person's disk, at the root and inside each project entry
    /// alike, and that is an ordinary state rather than damage: serde ignores a
    /// key it has no field for, in silence, so the file goes on loading and
    /// everything beside the retired key reads exactly as it did.
    ///
    /// Which is what makes removing a field cost no migration. A read that
    /// stripped the keys would be one-way code written for a value nobody reads,
    /// and it would take the setting away from somebody who went back to the
    /// previous version. There is no warning either — a key nobody asked for is
    /// not an error, and a log line per launch would outlive every reader of it.
    #[test]
    fn a_key_this_schema_no_longer_has_costs_its_neighbours_nothing() {
        let settings = settings_of(
            r#"{"version":1,"retired":{"level":"ultra"},"appearance":{"theme":"light"},
                "agent":"codex","projects":{"/a":{"retired":"lite","sideTab":"agents"}}}"#,
        );
        assert_eq!(settings.appearance.theme, "light", "a neighbouring section must survive");
        assert_eq!(settings.agent, "codex", "and so must a neighbouring field");
        assert_eq!(
            settings.projects["/a"].side_tab, "agents",
            "a project entry keeps the fields that are still fields"
        );
    }
