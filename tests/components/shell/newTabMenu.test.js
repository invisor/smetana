...
/* The + button's menu. Three rows, and the words are the point: this is the one
...
describe('the new-tab menu', () => {
  /* The order is the design and not an accident: the two rows that open a tab
     first, then the one that opens a dialog. Pinned exactly, so a fourth row
     appended anywhere but the end fails here rather than on somebody's screen. */
  it('offers exactly three things, an agent, a terminal and a task', () => {
    expect(NEW_TAB_ITEMS.map((item) => item.kind)).toEqual(['agent', 'terminal', 'task'])
    ...
    /* The glyph per row, and not merely that each row has one: `square-check`
       ...
       the row and the card it files agree on what a task looks like. */
    expect(NEW_TAB_ITEMS.map((item) => item.icon)).toEqual(['bot', 'terminal', 'square-check'])
  })
  ...
  it('has no row that cannot be picked', () => {
    for (const item of NEW_TAB_ITEMS) {
      ...
      // A glyph each, and all three are registered in core/icons.js.
      expect(['bot', 'terminal', 'square-check']).toContain(item.icon)
    }
  })
})
