...
export function usageLines(answer) {
  ...
  return [
    {
      name: windowName(answer.usage?.sessionLabel, 'Session'),
      value: limitLine(answer.usage?.sessionPct, answer.usage?.sessionReset)
    },
    {
      name: windowName(answer.usage?.weekLabel, 'This week'),
...
export function usageNote(answer, busy = false, error = null) {
  ...
  if (answer.state === 'unreadable') return unavailableNote(answer.reason)
  ...
  return unavailableNote(answer.reason)
}
